...
export function RuntimeTraces() {
  ...
  async function load() {
    ...
    try {
      const eventResponse = await fetch(`${apiUrl}/v1/traces/${encodeURIComponent(traceId)}`);
      if (!eventResponse.ok) throw new Error((await eventResponse.json()).error?.message ?? 'Trace not found');
      setEvents((await eventResponse.json()).events);
      const graphResponse = await fetch(`${apiUrl}/v1/evidence-graphs/${encodeURIComponent(traceId)}`);
  ...
  return <section className="memory-view">
    ...
    <article className="panel"><div className="section-head"><p className="overline">Events</p><h2>Sanitized runtime events</h2></div>{events.map((event) => <div className="permission" key={event.eventId}><span className="badge medium">{event.type}</span><div><strong>{event.target ?? event.actor}</strong><small>{new Date(event.timestamp).toLocaleString()} · {event.eventId}</small></div></div>)}</article>
